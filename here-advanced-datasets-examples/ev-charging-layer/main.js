(function () {
   ...
   */
  let showEvChargingPOI = map => {
    let service = platform.getPlatformDataService()


    let evProvider = new H.service.extension.platformData.TileProvider(service,
      {
        layerId: 'EVCHARGING_POI', level: 13
      }, 
      {
        resultType: H.service.extension.platformData.TileProvider.ResultType.MARKER,
      })
    
    let stations = new H.map.layer.TileLayer(evProvider)
