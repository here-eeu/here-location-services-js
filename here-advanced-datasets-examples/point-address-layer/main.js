(function () {
  ...
  let showPointAddress = map => {
    let service = platform.getPlatformDataService()

    let pointAddressProvider = new H.service.extension.platformData.TileProvider(service,
    {
      layerId: 'POINT_ADDRESS', level: 13
    }, {
        resultType: H.service.extension.platformData.TileProvider.ResultType.MARKER,
    })
    let point_address = new H.map.layer.MarkerTileLayer(pointAddressProvider)
    map.addLayer(point_address)
  }
